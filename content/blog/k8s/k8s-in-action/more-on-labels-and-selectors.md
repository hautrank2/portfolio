...
> **Nối tiếp [note 5.20](/blog/k8s/k8s-in-action/multiple-files-or-one).** File nằm
> trong thư mục `k8s/`, apply bằng `kubectl apply -f k8s/`.

[Note 5.17](/blog/k8s/k8s-in-action/labels-and-selectors) dùng label ở mức `key=value`. Còn
...
API, giống chuyện `v1` vs `apps/v1` ở [note 5.18](/blog/k8s/k8s-in-action/writing-a-service-file).
...
Để ý phép so ở [note 5.15](/blog/k8s/k8s-in-action/writing-a-deployment-file) vẫn đúng, chỉ
...
[Note 5.18](/blog/k8s/k8s-in-action/writing-a-service-file) đã cho thấy một Pod trần lọt vào
